...
## 5. Contributor accounts (Supabase) — KEYS ALREADY EMBEDDED

Your Supabase project is connected:

- Project URL: `https://algmkrcvnwhgpzexbgmr.supabase.co`
- Publishable key: embedded in `contributors.html`

**Two things left to do:**

1. **Create the table.** Supabase → SQL Editor → New query → paste the contents
   of `SUPABASE_SETUP.sql` (included in this package) → Run.

2. **Set the site URL.** Supabase → Authentication → URL Configuration →
   set Site URL to `https://moannotation.com`. Without this, confirmation and
   password-reset emails point to the wrong place.

Also check Authentication → Providers → Email has "Confirm email" switched on.

Then test: sign up on the live site with a personal email, confirm the email,
sign in, and check the dashboard shows your stage.

**Day to day:** open Table Editor → `contributors` and change someone's `stage`
to Screened, Verified, Assessed, Signed or Active. They see it next
time they sign in.

### Reference — original setup steps
...
`stage` to Screened, Verified, Assessed, Signed or Active. They see it
